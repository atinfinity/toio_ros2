...
|/toio/button|[std_msgs/msg/Bool](https://docs.ros2.org/foxy/api/std_msgs/msg/Bool.html)|`true` while the cube button is pressed. Published on change (the cube notifies changes only) and once on every (re)connection, with a `TRANSIENT_LOCAL` QoS so a late subscriber gets the current state. See the [toio spec](https://toio.github.io/toio-spec/docs/ble_button)|
...
|/toio/imu|[sensor_msgs/msg/Imu](https://docs.ros2.org/foxy/api/sensor_msgs/msg/Imu.html)|orientation of the cube every `imu_interval_ms`, in REP-103 axes (x forward, y left, z up) in the `center` frame. Only `orientation` is filled; `angular_velocity_covariance[0]` and `linear_acceleration_covariance[0]` are -1 to mark the other two absent. Roll and pitch are gravity-referenced. **Yaw is zero at the cube's power-on heading and drifts** (about 0.3 deg/s at rest on a real cube), so use the Position ID (`/toio/pose`) for heading; the yaw variance is set large accordingly. See the [toio spec](https://toio.github.io/toio-spec/docs/ble_high_precision_tilt_sensor)|
...
|/diagnostics|[diagnostic_msgs/msg/DiagnosticArray](https://docs.ros2.org/foxy/api/diagnostic_msgs/msg/DiagnosticArray.html)|node health every `diagnostic_updater.period` (1s), for `rqt_robot_monitor` and fleet-side monitoring. Three statuses under the node name: `connection` (ERROR while disconnected; reports the cube, docking and the button), `battery` (WARN at 20%, ERROR at 10%, WARN until the cube's first report), `position` (WARN while the Position ID is missed, while the cube is tilted, or before the first Position ID; reports x, y, horizontal and posture)|
...
|diagnostic_updater.period|double|1.0|publish period of `/diagnostics` in seconds (declared by `diagnostic_updater`)|
|stop_on_button|bool|false|use the cube button as a hold-to-stop: while it is pressed the motor gets a stop instead of `cmd_vel`, the same way as `stop_on_position_id_missed`. The newest `cmd_vel` is kept, so releasing the button resumes the motion. `goal_pose` / `dock_to_pose` are unaffected|
