...
|cube_id|string|''|connect only to the cube whose BLE name is `toio Core Cube-<cube_id>`|
...
To connect only to your own cube, set the `cube_id` parameter to the trailing
part of the cube's BLE local name `toio Core Cube-<cube_id>` (e.g. `C7f`).
The name of every cube found by the scan is printed in the node log at startup,
so you can find your `cube_id` there:

```
[INFO] [toio_ros2_node]: found cube: toio Core Cube-C7f (XXXXXXXX-...)
```

```bash
ros2 run toio_ros2 toio_ros2_node --ros-args -p cube_id:=C7f
```
